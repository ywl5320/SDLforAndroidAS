...
JNIEXPORT int JNICALL Java_org_libsdl_app_SDLActivity_nativeInit(JNIEnv* env, jclass cls, jstring jinput)
{
    int i;
    int argc;
    int status;
    ...
    const char* utf = NULL;
	if(jinput){
		utf = (*env)->GetStringUTFChars(env, jinput, 0);
		if(utf){
			argv[1] = SDL_strdup(utf);
			(*env)->ReleaseStringUTFChars(env, jinput, utf);
		}
	}
	argv[2] = NULL;
